c*************************************************************************
c     Variable Order Differential Equation (VODE) Solver
c*************************************************************************

      SUBROUTINE VODE (F, NEQ, Y, T, TOUT, ITOL, RTOL, ATOL, ITASK,
...
C-----------------------------------------------------------------------
C This is the 28 February 1991 version of
C VODE.. Variable-coefficient Ordinary Differential Equation solver,
C with fixed-leading coefficient implementation.
C This version is in real.
C
C VODE solves the initial value problem for stiff or nonstiff
C systems of first order ODEs,
C     dy/dt = f(t,y) ,  or, in component form,
C     dy(i)/dt = f(i) = f(i,t,y(1),y(2),...,y(NEQ)) (i = 1,...,NEQ).
C VODE is a package based on the EPISODE and EPISODEB packages, and
...
C Summary of usage.
C
C Communication between the user and the VODE package, for normal
...
C choices for MF (21, 22, 24, 25), and VODE requires the Jacobian
C matrix in some form.  In these cases (MF .gt. 0), VODE will use a
...
C or banded (MF = 24 or 25).  In the banded case, VODE requires two
...
C directly (MF = 21 or 24), but if this is not feasible, VODE will
...
C           REAL T, Y, PD, RPAR
C           DIMENSION Y(NEQ), PD(NROWPD,NEQ), RPAR(*), IPAR(*)
C
...
C
C D. Write a main program which calls subroutine VODE once for
C each point at which answers are desired.  This should also provide
C for possible use of logical unit 6 for output of error messages
C by VODE.  On the first call to VODE, supply arguments as follows..
...
C ISTATE = 2  if VODE was successful, negative otherwise.
...
C reset TOUT and call VODE again.  No other parameters need be reset.
C
C-----------------------------------------------------------------------
C EXAMPLE PROBLEM
C
...
C 60  FORMAT(/12H No. steps =,I4,12H   No. f-s =,I4,
C    1       12H   No. J-s =,I4,13H   No. LU-s =,I4/
C    2       28H  No. nonlinear iterations =,I4/
C    3       38H  No. nonlinear convergence failures =,I4/
C    4       27H  No. error test failures =,I4/)
C     STOP
...
C-----------------------------------------------------------------------
C Full description of user interface to VODE.
C
C The user interface to VODE consists of the following parts.
C
C i.   The call sequence to subroutine VODE, which is a driver
...
C
C ii.  Descriptions of other routines in the VODE package that may be
...
C
C iv.  Description of two routines in the VODE package, either of
...
C to the return from subroutine VODE to the user's calling program.)
C
...
C          externally to VODE, an extra call to F should be made
...
C
C RWORK  = A real working array (real).
...
C          (This will be checked by the solver.)
C
C Note..  The work arrays must not be altered between calls to VODE
C for the same problem, except possibly for the conditional and
C optional input, and except for the last 3*NEQ words of RWORK.
C The latter space is used for internal scratch space, and so is
C available for use by the user outside VODE between calls, if
...
C          or loaded arbitrarily, as they are overwritten by VODE.
...
C Optional Output.
C
C As optional additional output from VODE, the variables listed
C below are quantities related to the performance of VODE
C which are available to the user.  These are communicated by way of
C the work arrays, but also have internal mnemonic names as shown.
C Except where stated otherwise, all of this output is defined
C on any successful return from VODE, and on any return with
...
C                        defined only on a successful return from VODE.
C
C-----------------------------------------------------------------------
C Interrupting and Restarting
C
C If the integration of a given problem by VODE is to be
...
C the user should save, following the return from the last VODE call
...
C values before the next VODE call for that problem.  To save
C and restore the COMMON blocks, use subroutine VSRCO, as
...
C gain additional capabilities in conjunction with VODE.
C (The routines XSETUN and XSETF are designed to conform to the
C SLATEC error handling package.)
C
C     FORM OF CALL                  FUNCTION
C  CALL XSETUN(LUN)          Set the logical unit number, LUN, for
C                             output of messages from VODE, if
...
C
C  CALL VSRCO(RSAV,ISAV,JOB) Saves and restores the contents of
C                             the internal COMMON blocks used by
C                             VODE. (See Part iii below.)
C                             RSAV must be a real array of length 49
C                             or more, and ISAV must be an integer
C                             array of length 40 or more.
C                             JOB=1 means save COMMON into RSAV/ISAV.
C                             JOB=2 means restore COMMON from RSAV/ISAV.
C                                VSRCO is useful if one is
C                             interrupting a run and restarting
C                             later, or alternating between two or
C                             more problems solved with VODE.
C
C  CALL VINDY(,,,,,)         Provide derivatives of y, of various
C        (See below.)         orders, at a specified point T, if
C                             desired.  It may be called only after
C                             a successful return from VODE.
C
...
C             (normally the same as the T last returned by VODE).
...
C             by VODE directly.  Since NQCUR .ge. 1, the first
...
C If VODE is to be used in an overlay situation, the user
C must declare, in the primary overlay, the variables in..
C   (1) the call sequence to VODE,
C   (2) the two internal COMMON blocks
C         /VOD001/  of length  81  (48 real words
C                         followed by 33 integer words),
C         /VOD002/  of length  9  (1 real word
C                         followed by 8 integer words),
C
C If VODE is used on a system in which the contents of internal
...
C
C Below are descriptions of two routines in the VODE package which
...
C where NEQ, ITOL, RTOL, and ATOL are as in the VODE call sequence,
...
C VNORM routine (See below.), and also used by VODE in the computation
...
C     COMMON /VOD002/ HU, NCFN, NETF, NFE, NJE, NLU, NNI, NQU, NST
C     NQ = IVOD(28)
C     H = RVOD(21)
...
C value of VNORM suitable for use in the error control in VODE.
...
C-----------------------------------------------------------------------
C Other Routines in the VODE Package.
C
C In addition to subroutine VODE, the VODE package includes the
...
C  VNLSD    solves the underlying nonlinear system -- the corrector.
C  VJAC     computes and preprocesses the Jacobian matrix J = df/dy
C           and the Newton iteration matrix P = I - (h/l1)*J.
C  VSOL     manages solution of linear system in chord iteration.
...
C  VSRCO    is a user-callable routines to save and restore
C           the contents of the internal COMMON blocks.
C  DGEFA and DGESL   are routines from LINPACK for solving full
C           systems of linear algebraic equations.
C  DGBFA and DGBSL   are routines from LINPACK for solving banded
C           linear systems.
C  DAXPY, DSCAL, IDAMAX, DCOPY, and DDOT are basic linear algebra
C           modules (BLAS) used by LINPACK routines and/or this package.
...
C Note..  VNORM, IDAMAX, DDOT, D1MACH, LUNSAV, and MFLGSV are
C function routines.  All the others are subroutines.
C
C The intrinsic and external routines used by the VODE package are..
C ABS, AMAX1, AMIN1, DFLOAT, IABS, ISIGN, MAX0, MIN0, DSIGN, DSQRT,
C and WRITE.
C
C-----------------------------------------------------------------------
C
...
C communicated between subroutines in the VODE package, or which are
C to be saved between calls to VODE.
C In each block, real variables precede integers.
C The block /VOD001/ appears in subroutines VODE, VINDY, VSTEP, VSET,
C VNLSD, VJAC, VSOL, VJUST and VSRCO.
C The block /VOD002/ appears in subroutines VODE, VINDY, VSTEP, VNLSD,
C VJAC, and VSRCO.
C
...
C ICF    = Integer flag for convergence failure in VNLSD..
...
C-----------------------------------------------------------------------
 ...
 290  CONTINUE
C-----------------------------------------------------------------------
C CALL VSTEP (Y, YH, NYH, YH, EWT, SAVF, VSAV, ACOR,
C              WM, IWM, F, JAC, F, VNLSD, RPAR, IPAR)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
C Block G.
C The following block handles all successful returns from VODE.
...
C-----------------------------------------------------------------------
 ...
 602  MSG = 'VODE--   ITASK (=I1) illegal  '
      ...
      GO TO 700
 603  MSG='VODE--   ISTATE (=I1) .gt. 1 but VODE not initialized       '
...
C Function routines called by VHIN.. VNORM
C-----------------------------------------------------------------------
...
C
      NITER = 0
      TDIST = ABS(TOUT - T0)
      TROUND = UROUND*AMAX1(ABS(T0),ABS(TOUT))
...
C     /VOD002/ --  HU
C
C Subroutines called by VINDY.. DSCAL, XERRWV
...
C
C Subroutines called by VSTEP.. F, DAXPY, DCOPY, DSCAL,
C                               VJUST, VNLS, VSET
C Function routines called by VSTEP.. VNORM
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 ...
 120  IF (NEWQ .LE. MAXORD) GO TO 140
      ...
      NQ = MAXORD
      L = LMAX
 140  IF (KUTH .EQ. 1) ETA = AMIN1(ETA,ABS(H/HSCAL))
      IF (KUTH .EQ. 0) ETA = AMAX1(ETA,HMIN/ABS(HSCAL))
      ETA = ETA/AMAX1(ONE,ABS(HSCAL)*HMXI*ETA)
      NEWH = 1
      NQWAIT = L
...
C Compute ratio of new H to current H at the current order. ------------
      ...
      ETA = AMAX1(ETA,HMIN/ABS(H),ETAMIN)
...
C-----------------------------------------------------------------------
 530  IF (KFLAG .EQ. KFH) GO TO 660
      ...
      ETA = AMAX1(ETAMIN,HMIN/ABS(H))
      CALL VJUST (YH, LDYH, -1)
      L = NQ
      NQ = NQ - 1
      NQWAIT = L
      GO TO 150
 540  ETA = AMAX1(ETAMIN,HMIN/ABS(H))
      H = H*ETA
      HSCAL = H
      TAU(1) = H
...
C Test tentative new H against THRESH, ETAMAX, and HMXI, then exit. ----
 630  IF (ETA .LT. THRESH .OR. ETAMAX .EQ. ONE) GO TO 640
      ETA = AMIN1(ETA,ETAMAX)
      ETA = ETA/AMAX1(ONE,ABS(H)*HMXI*ETA)
      NEWH = 1
      HNEW = H*ETA
      GO TO 690
 640  NEWQ = NQ
...
C
C Subroutines called by VJUST.. DAXPY
...
C COMMON block variables accessed..
C     /VOD001/ ACNRM, CRATE, DRC, H, RC, RL1, TQ(5), TN, ICF,
C                JCUR, METH, MITER, N, NSLP
C     /VOD002/ HU, NCFN, NETF, NFE, NJE, NLU, NNI, NQU, NST
C
C Subroutines called by VNLSD.. F, DAXPY, DCOPY, DSCAL, VJAC, VSOL
C Function routines called by VNLSD.. VNORM
C-----------------------------------------------------------------------
C Subroutine VNLSD is a nonlinear system solver, which uses functional
C iteration or a chord (modified Newton) method.  For the chord method
C direct linear algebraic system solvers are used.  Subroutine VNLSD
C then handles the corrector phase of this integration package.
C
C Communication with VNLSD is done with the following variables. (For
...
C                 -1 convergence failure in previous call to VNLSD.
...
C to force VJAC to be called, if a Jacobian is involved.
C In any case, VJAC is called at least every MSBP steps.
C-----------------------------------------------------------------------
      DRC = ABS(RC-ONE)
...
C 2/(1+RC) to account for changes in h*rl1 since the last VJAC call.
C-----------------------------------------------------------------------
 350  DO 360 I = 1,N
...
C-----------------------------------------------------------------------
 400  IF (M .NE. 0) CRATE = AMAX1(CRDOWN*CRATE,DEL/DELP)
      DCON = DEL*AMIN1(ONE,CRATE)/TQ(4)
...
C COMMON block variables accessed..
C     /VOD001/  CCMXJ, DRC, H, RL1, TN, UROUND, ICF, JCUR, LOCJS,
C               MSBJ, NSLJ
C     /VOD002/  NFE, NST, NJE, NLU
C
C Subroutines called by VJAC.. F, JAC, DACOPY, DCOPY, DGBFA, DGEFA,
C                              DSCAL
C Function routines called by VJAC.. VNORM
C-----------------------------------------------------------------------
C VJAC is called by VSTEP to compute and process the matrix
...
C
C Communication with VJAC is done with the following variables.  (For
C more details, please see the comments in the driver subroutine.)
...
C If MITER = 2, make N calls to F to approximate the Jacobian. ---------
      ...
      R0 = THOU*ABS(H)*UROUND*DFLOAT(N)*FAC
...
C COMMON block variables accessed..
C     /VOD001/ -- H, RL1, MITER, N
C
C Subroutines called by VSOL.. DGESL, DGBSL
C Function routines called by VSOL.. None
C-----------------------------------------------------------------------
...
C Communication with VSOL uses the following variables..
...
C COMMON block variables accessed -- All of /VOD001/ and /VOD002/
C
C Subroutines/functions called by VSRCO.. None
C-----------------------------------------------------------------------
C This routine saves or restores (depending on JOB) the contents of the
C COMMON blocks VOD001 and VOD002, which are used internally by VODE.
C
...
C
c      COMMON /VOD001/ RVOD1(48), IVOD1(33)
c      COMMON /VOD002/ RVOD2(1), IVOD2(8)
...
C COMMON block variables accessed -- None
C
C Subroutines/functions called by EWSET.. None
C-----------------------------------------------------------------------
...
C COMMON block variables accessed -- None
C
C Subroutines/functions called by VNORM.. None
C-----------------------------------------------------------------------
...
C
 ...
 10     SUM = SUM + (V(I)*W(I))**2
      VNORM = SQRT(SUM/DFLOAT(N))
      RETURN
C----------------------- End of Function VNORM -------------------------
      END
      REAL FUNCTION D1MACH (IDUM)
      INTEGER IDUM
C-----------------------------------------------------------------------
...
C printing of the error messages by the error handling routine XERRWV.
C
...
C unit number to which error messages are printed by subroutine XERRWV.
C
C Saved local variable..
C
...
c
   ...
   50 continue
      ...
      subroutine jac(neqns,t,ya,ml,mu,pd,nrpd,rpar,ipar)
      real pd,rpar,t,ya
      dimension pd(nrpd,neqns),ya(neqns),rpar(*),ipar(*)


c     *****************************************************************
c     * a dummy jacobian routine for the VODE call
c     *****************************************************************
